...
def flag_hct_parcels(pipeline, parcels):
    ...
    """
    hct_stops = pipeline.get_geodataframe("hct_stops_2026")
    hct_stops = hct_stops[hct_stops["vision_hct"] == 1]
    #hct_stops = hct_stops[hct_stops["rural"] == 0]

    poly_list = []
    stop_types = ['srt', 'lrt', 'mrt', 'crt', 'fry', 'pof', 'brt']
    for stop_type in stop_types:
        stops = hct_stops[hct_stops[stop_type] == 1]
        buffer_distance = int(stops.vision_buffer.max())
...
def assign_ids(parcels, control_areas, rural_control_ids):
    ...
    parcels = parcels.sjoin(control_areas[["control_id", "geometry"]], how="left").drop(columns="index_right")
...
def run_step(context):
    ...
    parcels["tod"] = np.where(parcels["fry"] == 1, 5, parcels["tod"])   
    parcels["tod"] = np.where(parcels["pof"] == 1, 5, parcels["tod"])
    parcels["tod"] = np.where(parcels["crt"] == 1, 4, parcels["tod"])
    parcels["tod"] = np.where(parcels["mrt"] == 1, 4, parcels["tod"])      
    parcels["tod"] = np.where(parcels["lrt"] == 1, 4, parcels["tod"])         
    ...
    p.save_geodataframe('parcels_control_hct', parcels)
